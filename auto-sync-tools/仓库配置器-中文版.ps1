    ...
    try {
        ...
        if ($isGitRepo) {
            ...
            if ($LASTEXITCODE -eq 0) {
                $message = "检测到现有Git仓库，远程地址为：`n$currentRemote`n`n是否要更改为新的远程地址？"
                $result = [System.Windows.Forms.MessageBox]::Show($message, "确认", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question)
                if ($result -eq [System.Windows.Forms.DialogResult]::Yes) {
                    Push-Location $localPath
        ...
        $errorMessage = "测试连接时发生错误：" + $_.Exception.Message
        [System.Windows.Forms.MessageBox]::Show($errorMessage, "错误", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
...
$saveButton.Add_Click({
    ...
    try {
        ...
        $message = "配置保存成功！`n`n远程仓库: $remoteUrl`n本地路径: $localPath`n分支: $branch`n同步间隔: $interval 分钟"
        [System.Windows.Forms.MessageBox]::Show($message, "成功", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
        
    } catch {
        $errorMessage = "保存配置时发生错误：" + $_.Exception.Message
        [System.Windows.Forms.MessageBox]::Show($errorMessage, "错误", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
...
$startButton.Add_Click({
    ...
    try {
        ...
        if (Test-Path $scriptPath) {
            ...
            $message = "正在创建任务计划，请稍候...`n`n配置信息：`n远程仓库: $remoteUrl`n本地路径: $localPath`n分支: $branch`n同步间隔: $interval 分钟"
            [System.Windows.Forms.MessageBox]::Show($message, "成功", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
        } else {
        ...
        $errorMessage = "启动自动同步时发生错误：" + $_.Exception.Message
        [System.Windows.Forms.MessageBox]::Show($errorMessage, "错误", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
        $statusLabel.Text = "状态: 启动失败"
        $statusLabel.ForeColor = [System.Drawing.Color]::Red
    }
